//r para comer bayas normales
//a para comer bayas especiales
//n para dormir
//p para entrenamineto
...
image(cha_prin, mouseX-100, mouseY-100, 220, 200);
  if(dist(mouseX-100, mouseY-100, x, y) < 150 & key == 'r'){
    x = int(random(0, 1200));
    y = int(random(70, 700));
    i++;
    mostrartexto = false;
...
void bayaespecial(int w, int z){
  ...
  if(dist(mouseX-100, mouseY-100, w, z) < 200 & key =='a'){
    energia++;
    w = 2000;
    z = 2000;
    bayae++;
